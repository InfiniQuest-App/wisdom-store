...
const REFETCH_ELIGIBLE_TOOLS = new Set([
  ...
  'mcp__db-proxy__db_query',
  // Extend with your own read-only DB-query MCP tool names via env (comma-
  // separated), e.g. REFETCH_DB_QUERY_TOOLS="mcp__my-db__query,mcp__other__q".
  ...(process.env.REFETCH_DB_QUERY_TOOLS
    ? process.env.REFETCH_DB_QUERY_TOOLS.split(',').map(t => t.trim()).filter(Boolean)
    : [])
]);
